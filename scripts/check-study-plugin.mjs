...
const showcaseUrl = new URL('../src/components/HorizontalShowcase.jsx', import.meta.url)
const showcaseCssUrl = new URL('../src/components/HorizontalShowcase.css', import.meta.url)
...
assert.match(app, /src:\s*slug === 'StudyPlugin' \? studyPluginCover : coverModules\[k\]/)
assert.match(app, /fit:\s*slug === 'StudyPlugin' \? 'contain' : 'cover'/)
...
const showcase = readFileSync(showcaseUrl, 'utf8')
const showcaseCss = readFileSync(showcaseCssUrl, 'utf8')
...
assert.match(showcaseCss, /\.hs-card-media--contain img\s*\{[^}]*object-fit:\s*contain/s)
