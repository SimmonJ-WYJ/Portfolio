...
// Adapted from the shadcn/Tailwind "falling-pattern" component to this project's
// stack: plain JSX + inline styles. Animated falling light streaks broken up by a
// dotted overlay. `color` = streak color, `backgroundColor` = base/overlay color.
//
// Performance: the streaks animate via a plain CSS keyframe (cheap, GPU-scheduled)
// instead of framer-motion, and pause once the hero is scrolled away or the tab is
// hidden. The old full-viewport `backdrop-filter: blur()` was removed — it re-sampled
// the entire backdrop every frame and was the heaviest paint on the page.
export function FallingPattern({
  ...
  const idRef = useRef(`fp-${Math.round(Math.abs(Math.sin(duration) * 1e6))}`)

  useEffect(() => {
    // The hero is position:sticky, so it stays geometrically on-screen even when the
    // next section scrolls over it — an IntersectionObserver would never fire. Gate on
    // scroll position instead: pause once the hero is covered, and when the tab hides.
    const update = () => {
      setPaused(window.scrollY > window.innerHeight * 0.85 || document.hidden)
    }
    update()
  ...
  const anim = idRef.current
  return (
    <div className={`falling-pattern ${className}`.trim()} style={{ position: 'relative', height: '100%', width: '100%', padding: 1 }}>
      <style>{`@keyframes ${anim}{from{background-position:${startPositions}}to{background-position:${endPositions}}}`}</style>
      <div
        style={{
          position: 'relative',
          width: '100%',
          ...
          // Glow = blur + brightness applied directly to this isolated streak layer
          // (cheap, GPU-textured), reproducing the old look without a full-viewport
          // backdrop-filter (which re-sampled everything behind it every frame).
          filter: `blur(${blurIntensity}) brightness(${brightness})`,
